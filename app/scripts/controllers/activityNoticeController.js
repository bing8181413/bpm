...
define([
    ...
    * @params:
    */
    .controller('activityNoticeController', function (
        $http,
        $modal,
        $scope,
        $filter,
        ...
        $scope.Init = {
            ...
            count: parseInt($stateParams.count, 0),
            status: false
        };
        ...
        that.getData = function () {
            widget.ajaxRequest({
                scope: $scope,
                url: '/manage/push/activity/list',
                data: {
                    activity_id: $scope.Init.id,
        ...
        $scope.addNotice = function () {
            if (!$scope.Communitys) {
                widget.ajaxRequest({
                    scope: $scope,
                    url: '/manage/push/activity/communitys',
                    data: {
                        activity_id: $scope.Init.id
                    },
                    success: function (res) {
                ...
                var modalInstance = $modal.open({
                    ...
                    controller: function ($scope, $modalInstance) {
                        ...
                        $scope.ok = function () {
                            ...
                            var data = {
                                activity_id: that.Init.id,
                                title: $scope.tInput.title,
                                content: $scope.tInput.content,
                                //scope_type: $scope.tInput.scope_type,
                                noticeRange: $scope.tInput.noticeRange,
                                community_ids: community_ids,
                                order_status: $scope.tInput.order_status,
                                auto_send: auto_send,
                                send_time: send_time
                            };
                            ...
                            widget.ajaxRequest({
                                scope: $scope,
                                url: '/manage/push/activity/add',
                                data: data,
                                success: function (res) {
                                    widget.msgToast('活动通知添加成功');
                                    $scope.cancel();
                                    that.getData();
                                }
                            });
        ...
        if (!$scope.Init.count) $scope.addNotice();
