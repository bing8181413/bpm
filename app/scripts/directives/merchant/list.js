define([
  ...
  mod.directive('addMerchantProduct', function($templateCache, $filter, $compile, widget, $uibModal, $templateCache, comfunc) {
    return {
      multiElement: true,
      restrict: 'AE',
      replace: false,
          ...
          var modalInstance = $uibModal.open({
            ...
            controller: function($scope, $uibModalInstance) {
              ...
              $scope.submit = function() {
                if (!comfunc.isDecimal($scope.new_balance_rate) || !comfunc.isDecimal($scope.old_balance_rate)) {
                  widget.msgToast('分销比例填写数据范围不正确！');
                  return false;
                }
                ...
                $scope.param.products = $scope.products.map(function(item) {
                  return {
                    ...
                    new_balance_rate: $scope.new_balance_rate,
                    old_balance_rate: $scope.old_balance_rate,
                  };
                });
                widget.ajaxRequest({
    ...
    return {
      ...
      template: '<a class="btn btn-info btn-rounded btn-sm" ng-click="show()" ng-bind="data.products.count|null2empty" ng-show="data.stat.product_count"></a>',
      link: function($scope, $element, $attrs) {
        var supScope = $scope;
        $scope.show = function(status) {
          var modalInstance = $uibModal.open({
