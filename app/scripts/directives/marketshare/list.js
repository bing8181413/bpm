define([
    ...
    mod
        .directive('delMarketshareUser', function ($templateCache, $filter, $compile, widget) {
            return {
                multiElement: true,
                restrict: 'AE',
                replace: false,
                ...
                link: function ($scope, $element, $attrs) {
                    ...
                    $scope.change = function (status) {
                        if (confirm('确认修改为' + status_title + '状态?')) {
                            widget.ajaxRequest({
                                url: '/mobile/live/marketshares/' + $scope.data.market_share_id + '/mobiles/' + $scope.data.id,
                                method: 'delete',
                                scope: $scope,
                                data: {},
        ...
        .directive('marketshareImportUser', function ($rootScope, $templateCache, $filter, $compile, widget, $uibModal, $timeout) {
            return {
                restrict: 'AE',
                replace: false,
                scope: {
                        ...
                        var modalInstance = $uibModal.open({
                                ...
                                controller: function ($scope, $uibModalInstance) {
                                    ...
                                    $scope.submit = function () {
                                        ...
                                        widget.ajaxRequest({
                                            url: '/mobile/live/marketshares/' + (supscope.data.id || 0) + '/mobiles',
                                            method: 'POST',
                                            scope: $scope,
                                            data: $scope.param,
                                            success: function (json) {
